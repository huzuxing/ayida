$(document).ready(function() {
	...
	$(".tab").bind("click", function() {
		...
		$(this).parent().siblings().children("a").removeClass("select-hover");
	...
	var hospital_P = "";
	var hospital_S = "";
	var viceSenior = false;
	var degree = false;
	$(".item_select li").click(function() {
		var index = $(this).index();
		// key 是对再次点击的事件处理判断，0-表选选中 1-表示取消选中
		...
		if (0 == key) {
			...
			if ("专科医院" == value) {
				hospital_P = "专科医院";
				$("input[name=hospital_P]").val(hospital_P);
			}
			if ("三甲医院" == value) {
				hospital_S = "三甲医院";
				$("input[name=hospital_S]").val(hospital_S);
			}
			if ("副高以上" == value) {
				viceSenior = true;
				$("input[name=viceSenior]").val(viceSenior);
			}
			if ("学历优先" == value) {
				degree = true;
...
/**选择完城市后直接提交查询**/ 
function conditionalSubmit(cityName) {
	var query = $("input[name=query]").val().trim();
	var base = $("input[name=base]").val().trim();
	if ("all" == cityName) cityName = $(".select_tab li:eq(0)").text();
	window.location.href = base + "/api/search/web/search.jspx?query=" + query +"&cityName=" + cityName;
}
